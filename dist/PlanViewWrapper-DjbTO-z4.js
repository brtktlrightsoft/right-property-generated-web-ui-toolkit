...
var PlotRepository = class {
	...
	async fetchImage(e, t) {
		return this.makeGET(`asset/presignedurl/${e}?width=${t}&isThumbnail=false`);
	}
};
const plotRepository = new PlotRepository();
function createPlotRepository(e) {
	let t = new PlotRepository();
...
var PlanLabelObject = class extends fabric.Group {
	...
	initObject(e, t) {
		let n = e.width ?? 0, r = e.height ?? 0, i = scaleCoordinate(n, t.x), a = scaleCoordinate(r, t.y), o = scaleCoordinate(e.left, t.x), s = scaleCoordinate(e.top, t.y), c = o + i / 2, l = s + a / 2, u = Math.min(i, a) / 2, d = {
			objectId: e.objectId,
			...
			width: i,
			height: a,
			radius: e.radius ? scaleCoordinate(e.radius, t.x) : u,
			points: e.points ? e.points.map((e) => new fabric.Point(t.x * (e.x + n / 2), t.y * (e.y + r / 2))) : [],
			fill: e.fill,
			opacity: e.opacity,
			label: null
		};
		if (e.objectType === PlanObjectTypeEnumNames[PlanObjectTypeEnum.Rectangle]) {
			let e = new PlanRectangleObject(d);
			...
			let e = new PlanPolygonObject(d);
			return e.finishDrawing(), this.containerObjects.push(e), e;
		} else {
			let e = new PlanCircleObject(d);
			return e.finishDrawing(), this.containerObjects.push(e), e;
		}
	}
...
function scaleCoordinate(e, t) {
	return Number.isFinite(e) ? e >= 0 && e <= 1 ? e * t : e : 0;
}
var PlanItemController = class {
	get items() {
		return this._items;
	}
	constructor(e) {
...
function PlanViewPopup({ canvas: i, obj: a, item: o, onClickOutside: c }) {
	...
	let A = w?.name.toLowerCase() == "sold" ? l("web.availability.status.Sold") : formatCurrency(o.plotInfo?.price ?? 0, null, _, v, document.documentElement.lang);
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsx("div", { className: `absolute w-0 h-0 ${de(h[0], h[1])} ` })
		]
	});
}
...
var FloorViewPopup = React.forwardRef(({ canvas: e, obj: t, item: n, onClickOutside: r, onNavigate: i, measurementSystem: a = "metric" }, o) => {
	let c = useRef(null), [l, m] = useState([1, 1]), h = useRef(!1), g = (e) => {
...
function PlanView({ planId: e, objects: t, items: n, background: r, color: i, canvasSubject: a, useHalfWidth: c, onNavigate: l, formatCurrency: _, t: v, showPrice: y, measurementSystem: b }) {
	let x = useRef(null), S = useRef(null), C = useRef(null), w = useRef(null), T = useRef(null), E = useRef(null), D = useRef(null), O = useRef(0), k = useRef(0), A = useRef(!1), fe = useRef(!1), j = useRef(0), M = useRef(1), N = useRef(new PlanItemController(n)), P = useRef(new PlanObjectController(t)), F = useRef(t), I = useRef(""), L = useRef(null), R = useRef(0), z = useRef(0), B = useRef(0), V = useRef(null), [H, U] = useState([]), W = useRef(null), G = () => {
		let e = W.current;
		e && D.current.setDimensions({
			width: e.clientWidth,
		...
		if (S.current?.width !== 0 || S.current?.height !== 0) {
			let t = D.current.getWidth() / (c ? 2 : 1), n = S.current.width, r = S.current.height, i = D.current.getHeight(), a = t / n, o = i / r;
			e = Math.min(a, o), D.current.setZoom(e), D.current.absolutePan(new fabric.Point((n * e - t) / 2, (r * e - i) / 2)), M.current = e;
		}
		D.current.renderAll(), L.current = D.current?.viewportTransform;
	};
	useEffect(() => {
		let e = (e) => e.preventDefault(), t = () => K(), n = W.current ?? document, r = document?.getElementById("puck-canvas-root");
		...
		let { da: t } = e;
		e.event.preventDefault(), j.current == 0 && (j.current = t[0]);
		let n = t[0] / j.current * D.current.getZoom();
		n > 20 && (n = 20), n < M.current && (n = M.current), D.current.setZoom(n), Q(), j.current = t[0], D.current.renderAll();
	}, { target: V }), useEffect(() => {
		D.current && a && a.attach(() => {
			K();
		}, "onFit");
	}, [a, D.current]), useEffect(() => {
		if (!r?.objectUrl) return;
		let e = typeof window < "u" ? window.document : document;
		return e.addEventListener("gesturestart", (e) => e.preventDefault()), e.addEventListener("gesturechange", (e) => e.preventDefault()), q(), () => {
			try {
				D.current && (D.current?.dispose(), S.current?.dispose());
			} catch (e) {
				console.error(e);
			}
		};
		...
		let r = e == I.current;
		if (D.current && r) D.current.remove(...D.current.getObjects()), P.current.deleteObjects(), N.current = new PlanItemController(n), P.current = new PlanObjectController(t), P.current.initContainerObjects(C.current), P.current.containerObjects.forEach((e) => {
			let t = N.current.getItem(e.itemId);
			t && (e.assign(t), D.current.add(e, e.label));
		}), D.current?.requestRenderAll();
		else if (D.current && !r) try {
			D.current && (D.current.remove(...D.current.getObjects()), S.current?.dispose(), P.current.deleteObjects(), N.current = new PlanItemController(n), P.current = new PlanObjectController(t), q());
		} catch (e) {
			console.error(e);
		}
		F.current = t, I.current = e;
	}, [t]);
	let q = () => {
		fabric.Image.fromURL(r.objectUrl ?? "", (e) => {
			e.setOptions({
				...
				opacity: 1,
				hasBorders: !1
			}), S.current = e, C.current = new fabric.Point(S.current.width ?? 0, S.current.height ?? 0), pe(), _e(S.current?.width ?? 0, S.current?.height ?? 0), K();
		});
	}, J = (e) => {
		if (e.target instanceof PlanRectangleObject || e.target instanceof PlanCircleObject || e.target instanceof PlanPolygonObject) {
			let t = e.target, n = getPlanItemTypeEnum(N.current.getItem(t.itemId)?.type ?? "");
			n == PlanItemTypeEnum.Room && (E.current = n, X(t));
		}
	}, Y = (e) => {
		if ((e.target instanceof PlanRectangleObject || e.target instanceof PlanCircleObject || e.target instanceof PlanPolygonObject) && E.current == PlanItemTypeEnum.Room) {
		...
		S.current && (D.current, D.current = new fabric.Canvas(x.current, {
			hoverCursor: "move",
			selection: !0,
		...
		}), D.current.backgroundColor = i ?? "#ff00", D.current.selection = !1, D.current.defaultCursor = "grab", D.current.on("mouse:wheel", function(e) {
			let t = e.e.deltaY, n = D.current.getZoom(), r = new fabric.Point(D.current.getWidth() / 2, D.current.getHeight() / 2);
			n *= .999 ** t, n > 20 && (n = 20), n < M.current && (n = M.current);
			let i = D.current.getPointer(e.e), a = new fabric.Point(i.x, i.y), o = fabric.util.transformPoint(a, D.current.viewportTransform);
			D.current.zoomToPoint(r, n);
			let s = fabric.util.transformPoint(a, D.current.viewportTransform), c = new fabric.Point(o.x - s.x, o.y - s.y);
			D.current.relativePan(c), Q(), D.current.renderAll(), e.e.preventDefault(), e.e.stopPropagation();
		}), D.current.on("mouse:move", function(e) {
			A.current && (e.e.type == "mousemove" ? $(e.e.clientX, e.e.clientY) : e.e.touches && e.e.touches.length > 1 || $(e.e.touches[0].clientX, e.e.touches[0].clientY));
		}), D.current.on("mouse:up", ge), D.current.on("mouse:down", he), D.current.on("mouse:over", J), D.current.on("mouse:out", Y), P.current.initContainerObjects(C.current), P.current.containerObjects.forEach((e) => {
			let t = N.current.getItem(e.itemId);
			if (t) if (e.itemType == "Room") {
				t && e.assign(t);
				let n = e.fill, r = getObjectLabelFrame(e);
				e.fill = "#ff00", e.label = new PlanLabelObject(t.name, n ?? "0x000000", r.left, r.top, r.width, r.height, 1, 56), e.label.hideBackground(), e.label.addStroke(), D.current.add(e, e.label);
			} else t && e.assign(t), D.current.add(e, e.label);
		}), D.current.on("mouse:mouseup", (e) => {
			(e.target instanceof PlanRectangleObject || e.target instanceof PlanCircleObject || e.target instanceof PlanPolygonObject) && (X(e.target), D.current?.requestRenderAll());
		}), D.current.requestRenderAll(), fe.current = !0);
	}, me = async (e) => {
		...
		let t = N.current.getItem(e.itemId), n = getPlanItemTypeEnum(t?.type ?? "");
		switch (E.current = n, n) {
			case PlanItemTypeEnum.Plot:
				U([/* @__PURE__ */ jsx(PlanViewPopup, {
					canvas: D.current,
					obj: e,
					item: t,
					onClickOutside: () => {
			...
			case PlanItemTypeEnum.PlotContainer:
				U([/* @__PURE__ */ jsx(ContainerPlanPopup, {
					canvas: D.current,
					obj: e,
					item: t,
					onClickOutside: () => {
			...
			case PlanItemTypeEnum.Room:
				U([/* @__PURE__ */ jsx(floor_view_popup_default, {
					ref: w,
					canvas: D.current,
					obj: e,
					item: t,
		...
		E.current = null, U([]), T.current = null;
	}, [H]), he = (e) => {
		A.current = !0, e.e.type.includes("mouse") && (O.current = e.e.clientX, k.current = e.e.clientY), e.e.type.includes("touch") && (O.current = e.e.touches[0].clientX, k.current = e.e.touches[0].clientY), (e.target instanceof PlanRectangleObject || e.target instanceof PlanCircleObject || e.target instanceof PlanPolygonObject) && T.current && T.current.itemId == e.target.itemId ? me(T.current) : (e.target instanceof PlanRectangleObject || e.target instanceof PlanCircleObject || e.target instanceof PlanPolygonObject) && setTimeout(() => {
			X(e.target), D.current?.requestRenderAll();
		}, 150);
	}, ge = (e) => {
		A.current = !1, D.current?.fire("canvas:dragEnd"), j.current = 0;
	}, _e = (e, t) => {
		R.current = e, z.current = t;
	}, Q = () => {
		Date.now() - B.current < 8 || (B.current = Date.now(), ve(), D.current?.requestRenderAll());
	}, ve = () => {
		if (R.current === 0 || z.current === 0 || !D.current?.viewportTransform) return;
		let e = D.current?.viewportTransform, t = D.current?.getZoom(), n = D.current?.getWidth(), r = R.current, i = z.current, a = D.current?.getHeight(), o = L.current[4], s = L.current[5];
		e[4] >= L.current[4] ? D.current.viewportTransform[4] = L.current[4] : e[4] < n - r * t - o && (D.current.viewportTransform[4] = n - r * t - o), e[5] >= L.current[5] ? D.current.viewportTransform[5] = L.current[5] : e[5] < a - i * t - s && (D.current.viewportTransform[5] = a - i * t - s);
	}, $ = (e, t) => {
		if (D.current?.getZoom() == M.current) return;
		let n = new fabric.Point(e - O.current, t - k.current);
		D.current.relativePan(n), Q(), D.current?.requestRenderAll(), O.current = e, k.current = t;
	};
	return /* @__PURE__ */ jsx("div", {
		...
		children: /* @__PURE__ */ jsxs("div", {
			ref: V,
			className: "relative",
			children: [H.map((e) => createPortal(e, V.current)), /* @__PURE__ */ jsx("canvas", { ref: x })]
		})
	});
}
function getObjectLabelFrame(e) {
	let t = e.getScaledWidth(), n = e.getScaledHeight();
	return {
		left: (e.left ?? 0) - t / 2,
		top: (e.top ?? 0) - n / 2,
		width: t,
...
function PlanViewWrapper(e) {
	let { planId: t, objects: n, background: r, color: i, elementId: a = "canvas_container", useHalfWidth: o = !1, showPrice: c = !0, measurementSystem: l = "metric" } = e, [u, p] = useState(null), m = usePlotRepository();
	useEffect(() => {
...
export { plotRepository as _, ContainerPlanPopup as a, PlanObjectController as c, PlanCircleObject as d, PlanLabelObject as f, createPlotRepository as g, usePlotRepository as h, floor_view_popup_default as i, PlanPolygonObject as l, PlanItemTypeEnum as m, CanvasSubject as n, PlanViewPopup as o, PlanObjectTypeEnum as p, PlanView as r, PlanItemController as s, PlanViewWrapper as t, PlanRectangleObject as u };
